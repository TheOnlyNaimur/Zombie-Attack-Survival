...
def drawTrees():
    # Define tree positions - now placed near walls but slightly in front
    offset_from_wall = 50  # Distance in front of the wall
    tree_spacing = 170     # Space between trees
    ...
    for x, z in all_tree_positions:
        glPushMatrix()
        glTranslatef(x, 0, z)

        # Draw tree trunk (cylinder) - taller than walls
        glColor3f(0.55, 0.27, 0.07)  # Darkish brown
        glPushMatrix()
        glRotatef(-90, 1, 0, 0)  # Rotate to make cylinder vertical
        gluCylinder(gluNewQuadric(), 10, 10, 70, 10, 10)  # Height increased to 70
        glPopMatrix()

        # Draw tree top (sphere) - positioned above walls
        glColor3f(0.0, 0.5, 0.0)  # Green
        glPushMatrix()
        glTranslatef(0, 70, 0)  # Positioned at top of 70-unit trunk
